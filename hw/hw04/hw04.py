...
def balanced(m):
    ...
    right_arm=right(m)
    x = end(left_arm)
    if is_mobile(x) and (not balanced(x)):
        return False
    left_wt=total_weight(x)
    y = end(right_arm)
    if is_mobile(y) and (not balanced(y)):
        return False
    right_wt=total_weight(y)
    if length(left_arm)*left_wt==length(right_arm)*right_wt:
        return True
    else:
        return False
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y)!=0 and lower_bound(y)!=0, 'It\'s 0!'
