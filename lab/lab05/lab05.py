...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[(len(deck)//2-1)* (i%2) + (i+1)//2] for i in range(len(deck))]
...
def sprout_leaves(t, leaves):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
        new_t = tree(label(t),[tree(i) for i in leaves])
        return new_t
    else:
        new_t = tree(label(t),[sprout_leaves(branches(t)[i],leaves) for i in range(len(branches(t)))])
        return new_t
